...
if (isset($_POST['submit'])) {
    ...
    $data = [$_nama, $_tmp_lahir, $_tgl_lahir, $_gender, $_kategori, $_telpon, $_alamat, $_unit_kerja_id, $id];
    // Query SQL untuk update data pasien berdasarkan id
    $sql = "UPDATE paramedik SET nama = ?, tmp_lahir = ?, tgl_lahir = ?, gender = ?, kategori = ?, telpon = ?, alamat = ?, unit_kerja_id = ? WHERE id = ?";
    $stmt = $dbh->prepare($sql);
    $stmt->execute($data);
